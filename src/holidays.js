...
export class HolidayEvent extends Event {
  ...
  renderFullOrBasename(locale) {
    const str = l.lookupTranslation(this.getDesc(), locale);
    if (typeof str == 'string') return str;
    return l.lookupTranslation(this.basename(), locale);
  }
  /** @return {string} */
  url() {
...
export class RoshChodeshEvent extends HolidayEvent {
  ...
  render(locale) {
    ...
    return l.gettext(roshChodeshStr, locale) + ' ' + l.gettext(monthName, locale);
  }
  /** @return {string} */
  basename() {
    return this.getDesc();
...
export class MevarchimChodeshEvent extends Event {
  ...
  render(locale) {
    ...
    return l.gettext(mevarchimChodeshStr, locale) + ' ' + l.gettext(monthName, locale);
  }
  /**
   * Returns (translated) description of this event
   * @param {string} [locale] Optional locale name (defaults to active locale).
...
export const holidays = {
  ...
  getHolidaysForYear: function(year) {
    ...
    // first SAT after RH
    add(new HolidayEvent(new HDate(c.dayOnOrBefore(SAT, 7 + RH.abs())), 'Shabbat Shuva', SPECIAL_SHABBAT));
    addEvents(year, [
    ...
    add(
        new HolidayEvent(new HDate(c.dayOnOrBefore(SAT, pesachAbs - 43)), 'Shabbat Shekalim', SPECIAL_SHABBAT),
        new HolidayEvent(new HDate(c.dayOnOrBefore(SAT, pesachAbs - 30)), 'Shabbat Zachor', SPECIAL_SHABBAT),
    ...
    add(
        new HolidayEvent(new HDate(c.dayOnOrBefore(SAT, pesachAbs - 14) - 7), 'Shabbat Parah', SPECIAL_SHABBAT),
        new HolidayEvent(new HDate(c.dayOnOrBefore(SAT, pesachAbs - 14)), 'Shabbat HaChodesh', SPECIAL_SHABBAT),
        new HolidayEvent(new HDate(c.dayOnOrBefore(SAT, pesachAbs - 1)), 'Shabbat HaGadol', SPECIAL_SHABBAT),
        new HolidayEvent(
    ...
    add(new HolidayEvent(new HDate(c.dayOnOrBefore(SAT, new HDate(1, TISHREI, year + 1).abs() - 4)),
        'Leil Selichot', 0));
    ...
    add(
        new HolidayEvent(new HDate(c.dayOnOrBefore(SAT, av9dt.abs())), 'Shabbat Chazon', SPECIAL_SHABBAT),
        new HolidayEvent(av9dt.prev(), 'Erev Tish\'a B\'Av', 0, av9attrs),
        new HolidayEvent(av9dt, 'Tish\'a B\'Av', MAJOR_FAST, av9attrs),
        new HolidayEvent(new HDate(c.dayOnOrBefore(SAT, av9dt.abs() + 7)), 'Shabbat Nachamu', SPECIAL_SHABBAT),
    );
  ...
  getHolidaysOnDate: function(date) {
    ...
    const y = this.getHolidaysForYear(hd.getFullYear());
