 ...
 */
export function candleLightingEvents(holidaysYear, location, startAbs, endAbs) {
    let events = [];
...
export function hebcalEvents(options) {
    ...
    const beginHdate = isHebrewYear ?
        new HDate(1, theMonth || months.TISHREI, theYear) : new HDate(greg2abs(new Date(theYear, theMonth || 0, 1)));
    const startAbs = beginHdate.abs();
    const endAbs = isHebrewYear ? (
        options.month ? beginHdate.abs() + beginHdate.daysInMonth() : new HDate(1, months.TISHREI, theYear + 1)
    ) : (
        // Gregorian
        options.month ? beginHdate.abs() + greg.daysInMonth(theMonth + 1, theYear) : greg2abs(new Date(theYear + 1, 0, 1))
        );
    console.debug(`year=${theYear}, month=${theMonth}, isHebrewYear=${isHebrewYear}, startAbs=${startAbs}, endAbs=${endAbs}`);

    const allHolidays = holidays.year(beginHdate.getFullYear());
    let holidays0 = [];
    for (let abs = startAbs; abs <= endAbs; abs++) {
        const hd = new HDate(abs);
        const ev = allHolidays[hd];
        if (typeof ev !== 'undefined') {
            for (const e of ev) {
                if ((location.il && e.isIsraelOnly()) || (!location.il && e.isDiasporaOnly())) {
                    holidays0.push(e);
    ...
    console.log(`Got ${holidays0.length} from holidays`);
    let events = [];
    if (!options.noHolidays) {
        if (options.noModern) {
            console.log(`noModern - ${holidays0.length} before`);
            holidays0 = holidays0.filter(e => !(e.getFlags() & flags.MODERN_HOLIDAY));
            console.log(`noModern - ${holidays0.length} after`);
        }
        if (options.noRoshChodesh) {
            console.log(`noRoshChodesh - ${holidays0.length} before`);
            holidays0 = holidays0.filter(e => !(e.getFlags() & flags.ROSH_CHODESH));
            console.log(`noRoshChodesh - ${holidays0.length} after`);
        }
        if (options.noSpecialShabbat) {
            console.log(`noSpecialShabbat - ${holidays0.length} before`);
            ...
            console.log(`noSpecialShabbat - ${holidays0.length} after`);
        }
        events = events.concat(holidays0);
    }
    if (options.candlelighting) {
        const candleEvents = candleLightingEvents(allHolidays, location, startAbs, endAbs);
        console.log(`candleLighting - adding ${candleEvents.length}`);
        events = events.concat(candleEvents);
    }
    if (options.sedrot) {
        const sedra = new Sedra(beginHdate.getFullYear());
        for (let abs = startAbs; abs <= endAbs; abs++) {
            const dow = abs % 7;
            if (dow == 6) { // Saturday
                const hd = new HDate(abs);
                const parshaStr = sedra.getString(hd);
    ...
    if (options.dafyomi) {
        for (let abs = startAbs; abs <= endAbs; abs++) {
            ...
            events.push(new Event(new HDate(abs), dafyomi.dafname(dy), flags.DAF_YOMI));
        }
    }
    if (options.omer) {
