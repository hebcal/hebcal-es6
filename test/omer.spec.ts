...
test('getAnaBekoachWord', () => {
	...
	for (let i = 1; i <= 49; i++) {
		const ev = new OmerEvent(new HDate(startAbs + i - 1), i);
		const str = ev.getAnaBekoachWord();
		expect(str).toBe(expected[i]);
	}
});
...
test('getLamnatzeachLetter', () => {
	const expected = [
		'', 'י', 'ש', 'מ', 'ח', 'ו', 'ו', 'י', 'ר', 'נ', 'נ', 'ו', 'ל', 'א', 'מ', 'י', 'ם', 'כ', 'י', 'ת', 'ש', 'פ', 'ט', 'ע', 'מ', 'י', 'ם', 'מ', 'י', 'ש', 'ו', 'ר', 'ו', 'ל', 'א', 'מ', 'י', 'ם', 'ב', 'א', 'ר', 'ץ', 'ת', 'נ', 'ח', 'ם', 'ס', 'ל', 'ה'
	];
	const start = new HDate(16, 'Nisan', 5782);
	const startAbs = start.abs();
	for (let i = 1; i <= 49; i++) {
		const ev = new OmerEvent(new HDate(startAbs + i - 1), i);
		const str = ev.getLamnatzeachLetter();
		expect(str).toBe(expected[i]);
	}
});
...
test('getLamnatzeachWord', () => {
	...
	for (let i = 1; i <= 49; i++) {
		const ev = new OmerEvent(new HDate(startAbs + i - 1), i);
		const str = ev.getLamnatzeachWord();
		expect(str).toBe(expected[i]);
	}
});
