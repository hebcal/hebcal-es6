...
test('en', () => {
  ...
  expect(renderParshaName(['Vayakhel', 'Pekudei'], 'en')).toBe(
...
test('he', () => {
  ...
  expect(renderParshaName(['Vayakhel', 'Pekudei'], 'he')).toBe(
...
test('he-x-NoNikud', () => {
  ...
  expect(renderParshaName(['Vayakhel', 'Pekudei'], 'he-x-NoNikud')).toBe(
    'פרשת ויקהל־פקודי'
  );
});
