...
test('ParshaEvent-url', () => {
  const ev1 = new ParshaEvent({hdate: new HDate(new Date(2020, 4, 16)), parsha: ['Behar', 'Bechukotai'], il: false, num: [32, 33]});
  expect(ev1.url()).toBe('https://www.hebcal.com/sedrot/behar-bechukotai-20200516');
  const ev2 = new ParshaEvent({hdate: new HDate(new Date(2020, 5, 6)), parsha: ['Nasso'], il: false, num: 35});
  expect(ev2.url()).toBe('https://www.hebcal.com/sedrot/nasso-20200606');
  const ev3 = new ParshaEvent({hdate: new HDate(new Date(2020, 5, 13)), parsha: ['Beha\'alotcha']});
  expect(ev3.url()).toBe('https://www.hebcal.com/sedrot/behaalotcha-20200613');
  const ev4 = new ParshaEvent({hdate: new HDate(new Date(2022, 3, 30)), parsha: ['Achrei Mot']});
  expect(ev4.url()).toBe('https://www.hebcal.com/sedrot/achrei-mot-20220430');
  const ev5 = new ParshaEvent({hdate: new HDate(new Date(2022, 3, 30)), parsha: ['Kedoshim'], il: true});
...
test('getString-locale', () => {
  ...
  const ev = new ParshaEvent({hdate: hd, parsha: ['Pinchas'], il: false});
  expect(ev.render('he')).toBe('פָּרָשַׁת פִּינְחָס');

  const hd2 = new HDate(new Date(2020, 2, 21));
  const ev2 = new ParshaEvent({hdate: hd2, parsha: ['Vayakhel', 'Pekudei'], il: false});
  expect(ev2.render('he')).toBe('פָּרָשַׁת וַיַּקְהֵל־פְקוּדֵי');
});

test('early-ce-url', () => {
  const ev = new ParshaEvent({hdate: new HDate(new Date(100, 9, 16)), parsha: ['Bereshit']});
  expect(ev.url()).toBe('https://www.hebcal.com/sedrot/bereshit-01001016');
  const dt = new Date(99, 11, 12);
  dt.setFullYear(99);
  const ev2 = new ParshaEvent({hdate: new HDate(dt), parsha: ['Vayechi']});
  expect(ev2.url()).toBe(undefined);
});

test('bce-url', () => {
  const ev = new ParshaEvent({
    hdate: new HDate(new Date(-428, 8, 30)), parsha: ['Bereshit']});
