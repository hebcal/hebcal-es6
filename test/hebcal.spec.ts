...
test('no-holidays', () => {
  const events = calendar({noHolidays: true});
...
test('addHebrewDatesForEvents', () => {
  ...
  const ev0 = calendar(options0);
  expect(ev0).toHaveLength(0);

  const options1 = {year: 2017, month: 3};
  const ev1 = calendar(options1);
  expect(ev1).toHaveLength(8);
...
test('startAndEnd', () => {
  ...
  expect(ev1).toHaveLength(17);

  const eventsHDate = calendar({
    addHebrewDates: true,
  ...
  expect(eventsHDate).toHaveLength(17);

  const eventsAbsDate = calendar({
    addHebrewDates: true,
    start: 733319,
    end: 733359,
  });
  ...
  expect(eventsAbsDate).toHaveLength(56);
});
...
test('reformatTimeStr', () => {
  ...
  expect(
    reformatTimeStr('20:30', ' PM', {
      location: makeLocation('BR'),
    })
  ).toBe('8:30 PM');
  expect(
    reformatTimeStr('20:30', ' PM', {
      location: makeLocation('MX'),
    })
  ).toBe('20:30');

  expect(reformatTimeStr('11:45', 'pm', {})).toBe('11:45am');
  expect(
    reformatTimeStr('11:45', ' PM', {
      location: makeLocation('BR'),
    })
  ).toBe('11:45 AM');
  expect(
...
test('no-rosh-chodesh', () => {
  const events = calendar({year: 2020, noRoshChodesh: true});
...
test('no-minor-fast', () => {
  const events = calendar({year: 2020, noMinorFast: true});
  const ev = events.find(ev => ev.getDesc() == 'Tzom Gedaliah');
  expect(ev).toBe(undefined);
});

test('no-modern', () => {
  const events = calendar({year: 2020, noModern: true});
...
test('shabbat-mevarchim', () => {
  const events = calendar({year: 2020, shabbatMevarchim: true});
...
test('year0', () => {
  const events = calendar({year: 0});
  expect(events).toHaveLength(83);
});

test('version', () => {
  // the bare export is a string constant; HebrewCalendar.version() is a method
  expect(version.substring(0, version.indexOf('.'))).toBe('6');
  expect(HebrewCalendar.version()).toBe(version);
});

test('candlelighting-no-location-throw', () => {
  expect(() => {
    calendar({candlelighting: true});
...
test('bce', () => {
  ...
  const events = calendar(options).slice(0, 25);
  ...
  const expected = [
    ...
    {date: '-001539-09-10', desc: 'Shabbat Shuva'},
...
test('ykk with location copies attributes from src', () => {
  ...
  expect(ev.url()).toBeUndefined();
  expect(ev.memo).toBe(
    'Minor Day of Atonement on the day preceeding Rosh Chodesh Adar'
  );
...
test('tachanun', () => {
  ...
  expect(tachanun(erevRch, false)).toEqual({
    shacharit: true,
    mincha: false,
    allCongs: true,
  });
  const rch = new HDate(1, months.KISLEV, 5787);
  expect(tachanun(rch, true)).toEqual({
    shacharit: false,
    mincha: false,
    allCongs: false,
  });
...
test('Shabbat Mevarchim follows hour12 and locale', () => {
  ...
  expect(events3[0].memo).toBe(
    'מולד הלבנה סיון יהיה ביום רביעי בשבוע, בשעה 14 בצהריים, ו-12 דקות ו-10 חלקים'
  );
});
...
test('Eruv Tavshilin', () => {
  ...
  expect(HebrewCalendar.eruvTavshilin(new HDate(13, 'Nisan', 5785), true)).toBe(
    false
  );
  expect(HebrewCalendar.eruvTavshilin(new HDate(14, 'Nisan', 5785), true)).toBe(
    false
  );
  expect(HebrewCalendar.eruvTavshilin(new HDate(15, 'Nisan', 5785), true)).toBe(
    false
  );
  ...
  expect(HebrewCalendar.eruvTavshilin(new HDate(5, 'Sivan', 5785), false)).toBe(
    false
  );
  expect(HebrewCalendar.eruvTavshilin(new HDate(5, 'Sivan', 5785), true)).toBe(
    false
  );

  expect(HebrewCalendar.eruvTavshilin(new HDate(5, 'Sivan', 5786), false)).toBe(
    true
  );
  expect(HebrewCalendar.eruvTavshilin(new HDate(5, 'Sivan', 5786), true)).toBe(
    true
  );
...
test('yikzor', () => {
  ...
  const summarize = (ev: Event) => {
    const date = isoDateString(ev.greg());
    return {date, desc: ev.getDesc(), l: (ev as any).linkedEvent.getDesc()};
  };
  ...
  options.il = false;
  const diaspora = calendar(options);
  expect(diaspora).toHaveLength(4);
...
test('getHolidaysForYearArray', () => {
  ...
  expect(holidays).toHaveLength(99);
});

test('getHolidaysOnDate', () => {
  const holidays = getHolidaysOnDate(new Date(2020, 4, 29), false);
...
test('getSedra', () => {
  const sedra = getSedra(5781, false);
